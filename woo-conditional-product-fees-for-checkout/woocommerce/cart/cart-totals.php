	...
	<table cellspacing="0" class="shop_table shop_table_responsive">
		...
		foreach ( $fees as $fee ) :
			...
			<tr class="fee test">
				...
				<td data-title="<?php echo esc_attr( $fee->name ); ?>">
                    ...
                    // Get all the fee id
					$filtered_fee_title = apply_filters('wcpfc_all_fee_title', 'Fees');
					if ( has_filter('wcpfc_all_fee_title') ) {
						$filtered_fee_id = sanitize_title( $filtered_fee_title );
					} else {
						$filtered_fee_id = 'fees';
					}

					if( 0 === $fees_id && $filtered_fee_id === $fee->id ) {
		...
		if ( wc_tax_enabled() && ! WC()->cart->display_prices_including_tax() ) {
			...
			if ( WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping() ) {
				...
				$estimated_text = sprintf( ' <small>' . esc_html__( '(estimated for %s)', 'woocommerce-conditional-product-fees-for-checkout' ) . '</small>', WC()->countries->estimated_for_prefix( $taxable_address[0] ) . WC()->countries->countries[ $taxable_address[0] ] );
			}
